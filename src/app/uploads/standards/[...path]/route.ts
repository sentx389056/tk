...
export async function GET(_req: NextRequest, context: { params: Promise<{ path: string[] }> }) {
    try {
        ...
        // Динамический корень загрузок: UPLOADS_DIR или локальная папка uploads
        const uploadsRoot = process.env.UPLOADS_DIR || path.join(process.cwd(), 'uploads');
        const standardsRoot = path.join(uploadsRoot, 'standards') + path.sep;
        const filePath = path.resolve(path.join(standardsRoot, ...segments));

        // Запрещаем выход за пределы директории standards
        if (!filePath.startsWith(standardsRoot)) {
