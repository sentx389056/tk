...
export async function GET(_req: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    const meetingsRoot = path.join(uploadsRoot, 'meetings') + path.sep;
    const filePath = path.resolve(path.join(meetingsRoot, ...segments));
    if (!filePath.startsWith(meetingsRoot)) return new Response('Forbidden', { status: 403 });
