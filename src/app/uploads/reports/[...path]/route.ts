...
export async function GET(_req: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    const reportsRoot = path.join(uploadsRoot, 'reports') + path.sep;
    const filePath = path.resolve(path.join(reportsRoot, ...segments));
    if (!filePath.startsWith(reportsRoot)) return new Response('Forbidden', { status: 403 });
